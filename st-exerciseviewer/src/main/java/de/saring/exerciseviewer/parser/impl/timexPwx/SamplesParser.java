...
public class SamplesParser {
	...
	public EVExercise parseWorkoutSamples(EVExercise exercise, Node workoutNode) {
		...
		NodeList sampleNodes = null;
		sampleNodes = GetSampleNeodes(workoutNode);

		for (int nodeIndex = 0; nodeIndex < sampleNodes.getLength(); nodeIndex++) {
			Node sampleNode = sampleNodes.item(nodeIndex);
			...
			// Eliminates the jitters of 0bpm samples... assumes that heart rate won't
			// change instantiously by much and
			// that there will only be the occasional missed heart beat. Also fixes the laps
			// not adding up.
		...
		// some models (e.g. Timex Ironman Run Trainer) don't contain statistic date
		// (avg, max, ...)
	...
	private NodeList GetSampleNeodes(Node workoutNode) {
		XPathFactory xPathFactory = XPathFactory.newInstance();
		XPath xPath = xPathFactory.newXPath();
		try {
			XPathExpression xPathExpression = xPath.compile("//sample");
			return (NodeList) xPathExpression.evaluate(workoutNode, XPathConstants.NODESET);
	...
	private static void computeSpeedStatisticIfMissing(EVExercise exercise) {
		if (exercise.getRecordingMode().isSpeed() && exercise.getSpeed() == null) {
			...
			exSpeed.setSpeedAVG(CalculationUtils.calculateAvgSpeed(exSpeed.getDistance() / 1000f,
	...
	static int countNodeItems(Node node, String string2count) {
		// Given a Node and a Child Node Name, count the number of children with that
		// node name
